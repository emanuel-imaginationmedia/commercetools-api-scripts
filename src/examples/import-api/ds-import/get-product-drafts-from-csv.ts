import { Attribute, CategoryKeyReference, ProductDraftImport, ProductTypeKeyReference, ProductVariantDraftImport } from '@commercetools/importapi-sdk';
...
const getCategories = (categories: string): CategoryKeyReference[] => {
...
const getProductVariants = (productsFromCSV: Array<{ [key: string]: string }>, variants: string): ProductVariantDraftImport[] => {
    ...
    variants.split('|').forEach((productVariant) => {
        ...
        const productVariantFromCSV = productsFromCSV.find((productFromCSV) => {
            return productFromCSV['sku'] === propertiesObject['sku'];
        });
...
const convertProductFromCSVToProductDraftImport = (
    ...
    const productVariants = getProductVariants(productsFromCSV, productFromCSV['configurable_variations']);
    ...
    for (const productFromCSV of productsFromCSV) {
        try {
            if (productFromCSV['product_type'] === 'configurable' && productFromCSV['product_finish_type'] === 'Material') {
                productDrafts.push(convertProductFromCSVToProductDraftImport(productsFromCSV, productFromCSV));
